...
export default function App() {
  ...
  return (
    <>
      {/* Dashboard controls */}
      <VizInput name="petCount" label="Pet Count" inputType="number"
        value={petCount} setValue={setPetCount} min={1} max={10} />
      <VizInput name="nameLength" label="Name Length" inputType="number"
        value={nameLength} setValue={setNameLength} min={1} max={10} />
      <VizInput name="pwLength" label="Password Length" inputType="number"
        value={pwLength} setValue={setPwLength} min={8} max={64} />
      <VizInput name="prefix" label="Name Prefix" inputType="text"
        value={prefix} setValue={setPrefix} />

      <VizButton name="add-pet" label="Add Pet (+1)"
        handler={() => setPetCount((n: number) => Math.min(10, n + 1))} />
      <VizButton name="remove-pet" label="Remove Pet (-1)"
        handler={() => setPetCount((n: number) => Math.max(1, n - 1))} />
      <VizButton name="toggle-special"
      ...
      {Array.from({ length: petCount }, (_, i) => (
        <RandomPet key={`pet-${i}`} name={`${prefix}-pet-${i}`}
          length={nameLength} separator="-" />
      ))}

      <RandomString name={`${prefix}-password`}
        length={pwLength} special={pwSpecial} />
