...
export default function App() {
  ...
  return createElement(Fragment, null,
    // Viz controls
    createElement(VizInput, { name: "replicas", label: "Replicas", inputType: "number" as const, value: replicas, setValue: setReplicas, min: 1, max: 10 }),
    createElement(VizInput, { name: "instanceType", label: "Instance Type", inputType: "text" as const, value: instanceType, setValue: setInstanceType }),
    createElement(VizInput, { name: "region", label: "Region", inputType: "text" as const, value: region, setValue: setRegion }),
    createElement(VizInput, { name: "environment", label: "Environment", inputType: "text" as const, value: env, setValue: setEnv }),
    createElement(VizButton, { name: "scale-up", label: "Scale Up (+1)", handler: () => setReplicas((n: number) => Math.min(10, n + 1)) }),
    createElement(VizButton, { name: "scale-down", label: "Scale Down (-1)", handler: () => setReplicas((n: number) => Math.max(1, n - 1)) }),

    // Resources
    createElement(Vpc, { name: `${env}-vpc`, cidrBlock: "10.0.0.0/16", region },
