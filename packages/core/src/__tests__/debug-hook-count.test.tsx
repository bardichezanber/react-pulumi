...
import { loadState, prepareForRerender, resetState } from "../state-store.js";
  ...
  it("counts hooks and verifies state persistence across re-renders", () => {
    ...
    function App() {
      ...
      return createElement(
        ...
        createElement(
          Vpc,
          { name: "vpc", cidrBlock: "10.0.0.0/16" },
    ...
    for (const h of r1Hydrates) {
      console.log(
        `  index=${h.index} value=${JSON.stringify(h.value)} default=${JSON.stringify(h.defaultValue)}`,
      );
    }
